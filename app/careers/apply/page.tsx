...
export default function Apply(){
  ...
  const selectedPosition=requested && openRoles.some(role=>role.title===requested) ? requested : 'General Application';
  ...
  return <main className="page"><div className="shell application-shell"><span className="eyebrow">CAREERS</span><h1>{selectedPosition==='General Application'?'Join SMG.':`Apply for ${selectedPosition}.`}</h1><p className="lead">Tell us about yourself. Your application is for <strong>{selectedPosition}</strong>. Resume attachment is optional.</p>
    <form className="application-form" onSubmit={submit} encType="multipart/form-data">
      ...
      <div className="field"><label htmlFor="position">Position *</label><select id="position" name="position" value={selectedPosition} disabled><option>{selectedPosition}</option></select><input type="hidden" name="position" value={selectedPosition}/></div>
