...
const socialIcon=(platform:string)=>{const p=platform.toLowerCase(); if(p.includes('instagram'))return <Instagram size={18}/>; if(p.includes('linkedin'))return <Linkedin size={18}/>; if(p.includes('youtube'))return <Youtube size={18}/>; if(p.includes('facebook'))return <Facebook size={18}/>; if(p==='x'||p.includes('twitter'))return <Twitter size={18}/>; return <Globe size={18}/>};
