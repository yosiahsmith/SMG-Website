...
export type TeamMember = { name: string; email: string; title: string; position: string; pfp: string; overview: string; bio: string };
...
export default function TeamGrid({ members }: { members: TeamMember[] }) {
 ...
 return <>
  ...
  {selected&&<div className={styles.backdrop} onMouseDown={e=>e.target===e.currentTarget&&setSelected(null)}><div className={styles.modal} role="dialog" aria-modal="true" aria-label={`${selected.name} profile`}><button className={styles.close} onClick={()=>setSelected(null)} aria-label="Close profile">×</button><div className={styles.head}><span className={styles.modalPfp}>{selected.pfp?<img src={selected.pfp} alt=""/>:initials(selected.name)}</span><div><span className={styles.tag}>{selected.title}</span><div className={styles.modalPosition}>{selected.position}</div><h2>{selected.name}</h2><a href={`mailto:${selected.email}`}>{selected.email}</a></div></div><div className={styles.body}><p className={styles.overview}>{selected.overview}</p><p>{selected.bio}</p></div></div></div>}
 </>;
}
