...
export default function TeamGrid({ members }: { members: TeamMember[] }) {
 ...
 return <>
  <div className={styles.grid} aria-label="Solomon Media Group team">{members.map(m=><button key={m.email} type="button" className={styles.card} onMouseMove={move} onMouseLeave={leave} onClick={()=>setSelected(m)}><span className={styles.light} aria-hidden="true"/><span className={styles.pfp}>{m.pfp?<img src={m.pfp} alt=""/>:initials(m.name)}</span><span className={styles.info}><strong className={styles.name}>{m.name}</strong><span className={styles.email}>{m.email}</span></span></button>)}</div>
  {selected&&<div className={styles.backdrop} onMouseDown={e=>e.target===e.currentTarget&&setSelected(null)}><div className={styles.modal} role="dialog" aria-modal="true" aria-label={`${selected.name} profile`}><button className={styles.close} onClick={()=>setSelected(null)} aria-label="Close profile">×</button><div className={styles.head}><span className={styles.modalPfp}>{selected.pfp?<img src={selected.pfp} alt=""/>:initials(selected.name)}</span><div><span className={styles.tag}>{selected.title}</span><h2>{selected.name}</h2><a href={`mailto:${selected.email}`}>{selected.email}</a></div></div><div className={styles.body}><p className={styles.overview}>{selected.overview}</p><p>{selected.bio}</p></div></div></div>}
 </>;
}
