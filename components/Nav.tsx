...
export function Nav() {
  ...
  useEffect(() => {
    ...
    document.addEventListener('click', onClickOutside); document.addEventListener('keydown', onKey);
        ...
        <div className={styles.group}>
          <nav className={styles.links} aria-label="Primary">
            <div className={styles.item} ref={aboutRef} onMouseEnter={openAbout} onMouseLeave={closeAboutDelayed}>
              <Link href="/about" className={`${styles.link} ${aboutActive ? styles.active : ''}`} aria-haspopup="true" aria-expanded={aboutOpen} onClick={() => setAboutOpen(false)}>About <ChevronDown size={14} className={`${styles.chevron} ${aboutOpen ? styles.chevronOpen : ''}`} /></Link>
